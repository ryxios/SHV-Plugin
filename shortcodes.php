...
function getdb_data($dbwhere)
{
        ...
        if ($interval->format('%i') > $update) {
                ...
                } //$dbwhere == "ResultateTeams"
a:
                $datadb   = $wpdb->get_row("SELECT * FROM wp_shv_plugin WHERE name = '" . $dbwhere . "'");
                $data     = json_decode(json_encode($datadb), True);
                $response = $data['data'];
        } //$interval->format('%i') > $update
        else {
...
function create_spiele_list($atts, $result)
{
        ...
        foreach ($result as $gameData) {
                ...
                if ($olddate == "x") {
                        ...
                        $shvSpielplanoutput .= "<div class='shv-spiele-col'><span class='shv-spiele-titel'>Heim: </span></div>";
                        $shvSpielplanoutput .= "<div class='shv-spiele-col'><span class='shv-spiele-titel'>Gast: </span></div>";
                        if ($resultat != true) {
                        $shvSpielplanoutput .= "<div class='shv-spiele-col'><span class='shv-spiele-titel'>Zeit: </span></div>";
												$shvSpielplanoutput .= "<div class='shv-spiele-col'><span class='shv-spiele-titel'>Ort: </span></div>";
												} //$resultat === false
                ...
                if ($liga === true) {
                        $shvSpielplanoutput .= "<div class='shv-cell-container liga'><span class='shv-spiele-span'>Liga: </span><div class='shv-spiele-after'>" . $gameData["leagueShort"] . "</div></div>";
                } //$liga === true
                if ($resultat === true) {
                        $shvSpielplanoutput .= "<div class='shv-cell-container resultat'><span class='shv-spiele-span'>Resultat: </span><div class='shv-spiele-after'>" . $gameData["teamAScoreFT"] . ":" . $gameData["teamBScoreFT"] . " (" . $gameData["teamAScoreHT"] . ":" . $gameData["teamBScoreHT"] . ")</div></div>";
                } //$resultat === true
                $shvSpielplanoutput .= "<div class='shv-cell-container heim'><span class='shv-spiele-span'>Heim: </span><div class='shv-spiele-after'>" . $gameData["teamAName"] . "</div></div>";
                $shvSpielplanoutput .= "<div class='shv-cell-container gast'><span class='shv-spiele-span'>Gast: </span><div class='shv-spiele-after'>" . $gameData["teamBName"] . "</div></div>";
                if ($resultat != true) {
                $shvSpielplanoutput .= "<div class='shv-cell-container zeit'><span class='shv-spiele-span'>Zeit: </span><div class='shv-spiele-after'><span itemprop='startDate' content='" . $gameData['gameDateTime'] . "'>" . date("H:i", strtotime($time)) . "</div></div>";
                $shvSpielplanoutput .= "<div class='shv-cell-container ort'><span class='shv-spiele-span'>Ort: </span><div class='shv-spiele-after' itemprop='location' itemscope itemtype='http://schema.org/Place'>" . $venue . "</div></div>";
								} //$resultat === false
                $statlink = "https://www.handball.ch/de/matchcenter/spiele/" . $gameData["gameId"];
                $shvSpielplanoutput .= "<div class='shv-cell-container stats'><span class='shv-spiele-span'>Details: </span><div class='shv-spiele-after'><a href='" . $statlink . "' target='_blank' class='statslink'></a></div></div>";
                $shvSpielplanoutput .= "</div>";
                $olddate = $date;
        } //$result as $gameData
        if (empty($result)) {
...
function rangliste_shortcode($atts)
{
        ...
        foreach ($teams as $team) {
                ...
                if ($oldteamid == $teamId) {
                        ...
                        foreach ($result[$teamId]['ranking'] as $rangliste) {
                                ...
                                $output .= "<div class='rangliste-row " . $promstyle . "'>";
																...
																$output .= "<span class='rangliste-tore'>" . $rangliste['totalScoresPlus'] . "/" . $rangliste['totalScoresDiff'] . "/" . $rangliste['totalScoresMinus'] . "</span>";
                ...
                if ($result[$teamId]['groupText'] == "U13BeH-65") {
                        break;
                } //$result[$teamId]['groupText'] == "U13BeH-65"
        } //$teams as $team
        $output .= "</div>";
        return $output;
}
...
function spieler_stats_shortcode($atts)
{
        ...
        if ($err) {
                ...
                } //$dom->getElementsByTagName('tr') as $node
                ...
                } //$array2 as $key => $result
                foreach ($array3 as $spieler) {
                        $data3    = array_filter(explode(" ", strip_tags($spieler)), function($value)
                        {
                                return $value !== '';
                        });
                        $array4[] = $data3;
...
function kalenderurls_shortcode( )
{
        ...
        foreach ($teams as $team) {
                ...
                $output .= "</div>";
                if ($team['groupText'] == "U13ChQ-03") {
                        break;
                } //$team['groupText'] == "U13ChQ-03"
        } //$teams as $team
        $output .= "</div>";
        return $output;
}
